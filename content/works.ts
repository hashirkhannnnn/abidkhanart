/**
 * The catalogue — offline fallback.
 * ──────────────────────────────────────────────────────────────
 * Paintings are managed in WordPress and fetched by lib/wordpress.ts.
 * This file is the snapshot the site falls back to if WordPress cannot be
 * reached, so a CMS outage can never take the gallery down. Refresh it
 * with `npm run sync` after changes in WordPress.
 *
